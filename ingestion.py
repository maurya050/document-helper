...
from langchain_chroma import Chroma #store and index data locally
...
#chroma = Chroma(persist_directory = "chroma_db", embedding_funtion=embeddings)
...
async def index_documents_async(documents: List[Document], batch_size: int = 50):
    ...
    #Process all baches cocurrently
    async def add_batch(batch: List[Document], batch_num: int):
        try:
            await vectorstore.aadd_documents(batch)
            log_success(
                f"VectorSotre Inexing: Successfully added batch {batch_num}/{len(batches)} ({len(batch)} documents)"
            )
    ...
    if successful == len(batches):
        log_success(
            f"VectorStore indesxing: All batches processed successfully! ({successful}/{len(batches)})"
        )
    else:
        log_warning(
...
async def main():
    ...
    log_header("DOCUMNET CHUNKING PHASE")
    log_info(
    ...
    log_info(f"  * URLs crawled: {len(res['results'])}")
    log_info(f"  * Document Extracted: {len(all_docs)}")
